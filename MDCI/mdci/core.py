...
import numpy as np
import pdb
from ._dci import _dci_new, _dci_get_proj_vec, _dci_get_num_points, _dci_get_num_levels, _dci_add,  _dci_delete,_dci_query, _dci_clear, _dci_reset, _dci_add_query
...
class DCI(object):
    ...
    def __init__(self, dim, num_comp_indices = 2, num_simp_indices = 7, max_num_points = 0, lbd = 0.5, init = False):
        ...
        if init:
            self._dci_inst = _dci_new(dim, num_comp_indices, num_simp_indices, lbd, max_num_points)
    ...
    def _check_array(self, arr):
        ...
        if arr.dtype != np.float32:
            raise TypeError("array must consist of single-precision floats")
    ...
    def add(self, data, mask, data_ids, indices = None, num_levels = 2, blind = False, num_to_visit = -1, num_to_retrieve = -1, prop_to_visit = -1.0, prop_to_retrieve = -1.0, field_of_view = 10):
        ...
        if is_contiguous:
            #pdb.set_trace()
            _dci_add(self._dci_inst, data, mask, data_ids, num_levels, blind, num_to_visit, num_to_retrieve, prop_to_visit, prop_to_retrieve, field_of_view)
        else:
            selected_data = data[_indices]
            _dci_add(self._dci_inst, selected_data, mask, data_ids, num_levels, blind, num_to_visit, num_to_retrieve, prop_to_visit, prop_to_retrieve, field_of_view)
    ...
    def delete(self, data_ids, field_of_view = 10, blind = False, num_to_visit = -1, num_to_retrieve = -1, prop_to_visit = -1.0, prop_to_retrieve = -1.0, num_dci = 1):
        ...
        _dci_delete(self._dci_inst, data_ids, blind, num_to_visit, num_to_retrieve, prop_to_visit, prop_to_retrieve, field_of_view)

    
    # query is num_queries x dim
    def query(self, query, mask, num_neighbours = -1, field_of_view = 100, blind = False, num_to_visit = -1, num_to_retrieve = -1, prop_to_visit = -1.0, prop_to_retrieve = -1.0, scale = 1.0):
        ...
        _nearest_neighbour_idx, _nearest_neighbour_dists, num_candidates = _dci_query(self._dci_inst, _query, mask, num_neighbours, blind, num_to_visit, num_to_retrieve, prop_to_visit, prop_to_retrieve, field_of_view, scale)
        ...
        j = 0
        for i in range(_query.shape[0]):
            nearest_neighbour_num.append(num_candidates[i])
            j += num_candidates[i]
        
        return nearest_neighbour_idx, nearest_neighbour_dists, nearest_neighbour_num

    def add_query(self, key, query, value, mask, num_inst = 1, indices = None, num_levels = 2, num_points = 0, blind = False, c_num_to_visit = -1, c_num_to_retrieve = -1, 
                            c_prop_to_visit = -1.0, c_prop_to_retrieve = -1.0, c_field_of_view = 10, num_neighbours = -1,  q_field_of_view = 100, 
                            q_num_to_visit = -1, q_num_to_retrieve = -1, q_prop_to_visit = -1.0, q_prop_to_retrieve = -1.0, scale = 1.0, parallel_level = 1,
                            causal = False, attention_mask = None):
        
        data_ids = np.array([]).astype(np.float32)  # not used in IceFormer

        if num_levels >= 3:
            self._ensure_positive_integer(c_field_of_view)
        else:
            c_field_of_view = -1
        ...
        if num_levels >= 2:
            self._ensure_positive_integer(q_field_of_view)
        else:
            q_field_of_view = -1
        ...
        # num_queries x num_neighbours
        if is_contiguous:
            new_value = _dci_add_query(key, data_ids, _query, _value, _mask, num_inst, num_levels, num_neighbours, blind, c_num_to_visit, q_num_to_visit, c_num_to_retrieve, 
                                                                                                                ...
                                                                                                                self._lbd, num_points, scale, parallel_level, attention_mask)
        else:
            selected_data = key[_indices]
            new_value = _dci_add_query(selected_data, data_ids, _query, _value, _mask, num_inst, num_levels, num_neighbours, blind, c_num_to_visit, q_num_to_visit, c_num_to_retrieve, 
                                                                                                                q_num_to_retrieve, c_prop_to_visit, q_prop_to_visit, c_prop_to_retrieve, q_prop_to_retrieve, 
                                                                                                                c_field_of_view, q_field_of_view, self.num_comp_indices, self.num_simp_indices, 
                                                                                                                self._lbd, num_points, scale, parallel_level, attention_mask)
            self._orig_indices = _indices
