...
static bool btree_i_insert_descend(btree_i* const tree, void* n, const float key, const int value, float* splitkey, void** splitnode);
static void btree_i_split_leaf(btree_i* const tree, btree_i_leaf_node* leaf, float* _newkey, void** _newleaf);
static void btree_i_split_inner(btree_i* const tree, btree_i_inner_node* inner, float* _newkey, void** _newinner, unsigned int addslot);
static int btree_i_find_lower_inner(const btree_i_inner_node *n, const float key);
static int btree_i_find_lower_leaf(const btree_i_leaf_node *n, const float key);
static void btree_i_free_node(void *n);
static bool btree_i_is_leaf(const void*n);
static unsigned short btree_i_num_slots_used(void *n);

static float* btree_i_copy_keys_fwd(float* first, float* last, float* d_first);
static float* btree_i_copy_keys_bwd(float* first, float* last, float* d_last);
static int* btree_i_copy_data_fwd(int* first, int* last, int* d_first);
static int* btree_i_copy_data_bwd(int* first, int* last, int* d_last);
static void** btree_i_copy_child_fwd(void** first, void** last, void** d_first);
static void** btree_i_copy_child_bwd(void** first, void** last, void** d_last);

static btree_c_result btree_i_delete_descend(btree_i* const tree, const float key, const int value, void *curr, void *left, void *right, btree_i_inner_node *leftparent, btree_i_inner_node *rightparent, btree_i_inner_node *parent, unsigned int parentslot);
static btree_c_result btree_i_merge_leaves(btree_i* const tree, btree_i_leaf_node* left, btree_i_leaf_node* right, btree_i_inner_node* parent);
static btree_c_result btree_i_merge_inner(btree_i_inner_node* left, btree_i_inner_node* right, btree_i_inner_node* parent, unsigned int parentslot);
static btree_c_result btree_i_shift_left_leaf(btree_i_leaf_node *left, btree_i_leaf_node *right, btree_i_inner_node *parent, unsigned int parentslot);
static void btree_i_shift_left_inner(btree_i_inner_node *left, btree_i_inner_node *right, btree_i_inner_node *parent, unsigned int parentslot);
static void btree_i_shift_right_leaf(btree_i_leaf_node *left, btree_i_leaf_node *right, btree_i_inner_node *parent, unsigned int parentslot);
static void btree_i_shift_right_inner(btree_i_inner_node *left, btree_i_inner_node *right, btree_i_inner_node *parent, unsigned int parentslot);
...
static btree_i_leaf_node* btree_i_allocate_leaf()
{
    btree_i_leaf_node *n = (btree_i_leaf_node *)malloc(sizeof(btree_i_leaf_node));
    n->prev_leaf = n->next_leaf =  NULL;
    n->level = 0;
    n->num_slots_used = 0;
    n->slot_keys = malloc(sizeof(float)*BTREE_LEAF_MAX_NUM_SLOTS);
    n->slot_data = malloc(sizeof(int)*BTREE_LEAF_MAX_NUM_SLOTS);

    return n;
}

// Allocate and initialize an inner node
static btree_i_inner_node* btree_i_allocate_inner(unsigned short level)
{
    btree_i_inner_node *n = (btree_i_inner_node *)malloc(sizeof(btree_i_inner_node));
    n->level = level;
    n->num_slots_used = 0;
...
static bool btree_i_is_below_min(const btree_i* const tree, const void*n)
{
    if(btree_i_is_leaf(n))
    {
        btree_i_leaf_node* nn = (btree_i_leaf_node*)n;
        return (nn->num_slots_used < tree-> leaf_min_num_slots);
    }
    else
    {
        btree_i_inner_node* nn = (btree_i_inner_node*)n;
        return (nn->num_slots_used < tree->inner_min_num_slots);
    }
}

// True if few used entries, less than half full
static bool btree_i_is_few(const btree_i* const tree, const void*n)
{
    if(btree_i_is_leaf(n))
    {
        btree_i_leaf_node* nn = (btree_i_leaf_node*)n;
...
static unsigned short btree_i_num_slots_used(void *n){
    if (btree_i_is_leaf(n)) {
        btree_i_leaf_node *ln = (btree_i_leaf_node*)(n);
        return ln->num_slots_used;

    }
    else {
        btree_i_inner_node *in = (btree_i_inner_node*)(n);
...
// bulk_load().
static void btree_i_set_slot(btree_i_leaf_node* n, unsigned short slot, const float key, const int value)
{
...
static bool btree_i_is_full(const btree_i* const tree, const void*n)
{
    if(btree_i_is_leaf(n))
    {
        btree_i_leaf_node* nn = (btree_i_leaf_node*)n;
...
// places in btree_i_leaf_node and btree_i_inner_node.
static int btree_i_find_lower_inner(const btree_i_inner_node *n, const float key) 
{
...
bool btree_i_insert(btree_i* const tree, const float key, const int value)
{
    ...
    if (tree->root == NULL) {
        tree->root = tree->first_leaf = tree->last_leaf = btree_i_allocate_leaf();
    }

    bool r = btree_i_insert_descend(tree, tree->root, key, value, &newkey, &newchild);

    if (newchild)
    {
        btree_i_inner_node *newroot = btree_i_allocate_inner(*(unsigned short*)tree->root + 1);//tree->root->level+1
...
static bool btree_i_insert_descend(btree_i* const tree, void* n, const float key, const int value, float* splitkey, void** splitnode)
{
    if (!btree_i_is_leaf(n))
    {
        btree_i_inner_node *inner = (btree_i_inner_node*)(n);

        float newkey;
        void *newchild = NULL;

        int slot = btree_i_find_lower_inner(inner, key);
#ifdef BTREE_DEBUG 
        printf("btree::insert_descend into 0x%x\n", inner->slot_ptrs[slot]);
#endif
        bool r = btree_i_insert_descend(tree, inner->slot_ptrs[slot],
        ...
        if (newchild)
        {
...
#endif
            if (btree_i_is_full(tree,inner))
            {
                btree_i_split_inner(tree, inner, splitkey, splitnode, slot);

                if (slot == inner->num_slots_used+1 && inner->num_slots_used < btree_i_num_slots_used(*splitnode))
                {
                    ...
                    btree_i_inner_node *splitinner = (btree_i_inner_node*)(*splitnode);
                ...
                else if (slot >= inner->num_slots_used+1)
                {
                    ...
                    inner = (btree_i_inner_node*)(*splitnode);
                                    }
            }
            ...
            btree_i_copy_keys_bwd(inner->slot_keys + slot, inner->slot_keys + inner->num_slots_used,
                               inner->slot_keys + inner->num_slots_used+1);
            btree_i_copy_child_bwd(inner->slot_ptrs + slot, inner->slot_ptrs + inner->num_slots_used+1,
    ...
    else // n->btree_i_is_leaf() == true
    {
        btree_i_leaf_node *leaf = (btree_i_leaf_node*)(n);

        int slot = btree_i_find_lower_leaf(leaf, key);

        if (!allow_duplicates && slot < leaf->num_slots_used && key == leaf->slot_keys[slot]) {
            return false;
        }

        if (btree_i_is_full(tree, leaf))
        {
            btree_i_split_leaf(tree, leaf, splitkey, splitnode);

            // check if insert slot is in the split sibling node
            if (slot >= leaf->num_slots_used)
            {
                slot -= leaf->num_slots_used;
                leaf = (btree_i_leaf_node*)(*splitnode);
            }
        }
        ...
        btree_i_copy_keys_bwd(leaf->slot_keys + slot, leaf->slot_keys + leaf->num_slots_used,
                           leaf->slot_keys + leaf->num_slots_used+1);
        btree_i_copy_data_bwd(leaf->slot_data + slot, leaf->slot_data + leaf->num_slots_used,
...
static void btree_i_split_leaf(btree_i* const tree, btree_i_leaf_node* leaf, float* _newkey, void** _newleaf)
{
    assert(btree_i_is_full(tree, leaf));

    unsigned int mid = (leaf->num_slots_used >> 1);

    
    btree_i_leaf_node *newleaf = btree_i_allocate_leaf();
    ...
    btree_i_copy_keys_fwd(leaf->slot_keys + mid, leaf->slot_keys + leaf->num_slots_used,
              newleaf->slot_keys);
    btree_i_copy_data_fwd(leaf->slot_data + mid, leaf->slot_data + leaf->num_slots_used,
              newleaf->slot_data);
...
static void btree_i_split_inner(btree_i* const tree, btree_i_inner_node* inner,float* _newkey, void** _newinner, unsigned int addslot)
{
    assert(btree_i_is_full(tree, inner));
    ...
    btree_i_inner_node *newinner = btree_i_allocate_inner(inner->level);

    newinner->num_slots_used = inner->num_slots_used - (mid + 1);

    btree_i_copy_keys_fwd(inner->slot_keys + mid+1, inner->slot_keys + inner->num_slots_used,
              newinner->slot_keys);
    btree_i_copy_child_fwd(inner->slot_ptrs + mid+1, inner->slot_ptrs + inner->num_slots_used+1,
              newinner->slot_ptrs);
...
typedef struct{
    btree_i_inner_node* first;
    const float* second;
} btree_i_next_level_info;
...
void btree_i_bulk_load(btree_i* const tree, float* keybegin, float* keyend, int* databegin, int* dataend)
{
    ...
    btree_i_next_level_info* nextlevel = malloc(sizeof(btree_i_next_level_info)*num_parents);

    btree_i_leaf_node* leaf = tree->first_leaf;
    for (size_t i = 0; i < num_parents; ++i)
    {
        // allocate new inner node at level 1
        btree_i_inner_node* n = btree_i_allocate_inner(1);
    ...
    for (int level = 2; num_parents != 1; ++level)
    {
        ...
        for (size_t i = 0; i < num_parents; ++i)
        {
            ...
            btree_i_inner_node* n = btree_i_allocate_inner(level);
...
static inline bool btree_i_result_has(btree_c_result src, btree_c_result_flags f)
{
...
static int btree_i_delete_find_match(const btree_i_leaf_node *n, const float key, const int value)
{
    float keylow = key - 1e-8;
    float keyhi  = key + 1e-8;
    int lo = 0;
    if (sizeof(n->slot_keys) > BTREE_BIN_SEARCH_THRESH)
    {
        if (n->num_slots_used == 0) return 0;
    ...
    while(lo < n->num_slots_used && n->slot_keys[lo]<=keyhi ){
        if(n->slot_data[lo] == value) return lo;
        lo++;
    }
    return -1;
}
...
static inline void btree_i_delete_merge_result(btree_c_result* dst, const btree_c_result other) 
{
    dst->flags = dst->flags | other.flags;

    // we overwrite existing last_keys on purpose
    if (btree_i_result_has(other, btree_update_last_key))
...
bool btree_i_delete(btree_i* const tree, const float key, const int value)
{
    ...
    btree_c_result result = btree_i_delete_descend(tree, key, value, tree->root, NULL, NULL, NULL, NULL, NULL, 0);

    return !btree_i_result_has(result, btree_not_found);
}
...
// to a leaf_node_p, which means that this function must do a recursive depth
...
static btree_c_result btree_i_delete_descend(btree_i* const tree, const float key, const int value, void *curr, void *left, void *right, btree_i_inner_node *leftparent, btree_i_inner_node *rightparent, btree_i_inner_node *parent, unsigned int parentslot)
{
    if (btree_i_is_leaf(curr))
    {
        btree_i_leaf_node *leaf = (btree_i_leaf_node*)(curr);
        btree_i_leaf_node *leftleaf = (btree_i_leaf_node*)(left);
        btree_i_leaf_node *rightleaf = (btree_i_leaf_node*)(right);

        int slot = btree_i_delete_find_match(leaf,key,value);
        if(slot == -1)
        {
            return btree_c_result_of(btree_not_found, 0 );
        }
        
        btree_i_copy_keys_fwd(leaf->slot_keys + slot+1, leaf->slot_keys + leaf->num_slots_used,
                  leaf->slot_keys + slot);
        btree_i_copy_data_fwd(leaf->slot_data + slot+1, leaf->slot_data + leaf->num_slots_used,
                  leaf->slot_data + slot);
        ...
        if (slot == leaf->num_slots_used)
        {
            ...
            else
            {
                if (leaf->num_slots_used >= 1)
                {
                    ...
                    btree_i_delete_merge_result(&myres, btree_c_result_of(btree_update_last_key, leaf->slot_keys[leaf->num_slots_used - 1]));
                }
                else
                {
        ...
        if (btree_i_is_below_min(tree, leaf) && !(leaf == tree->root && leaf->num_slots_used >= 1))
        {
            ...
            // with our id
            else if ( (leftleaf == NULL || btree_i_is_few(tree, leftleaf)) && (rightleaf == NULL || btree_i_is_few(tree, rightleaf)) )
            {
                if (leftparent == parent)
                    btree_i_delete_merge_result(&myres, btree_i_merge_leaves(tree, leftleaf, leaf, leftparent));
                else
                    btree_i_delete_merge_result(&myres, btree_i_merge_leaves(tree, leaf, rightleaf, rightparent));
            }
            // case : the right leaf has extra data, so balance right with current
            else if ( (leftleaf != NULL && btree_i_is_few(tree, leftleaf)) && (rightleaf != NULL && !btree_i_is_few(tree, rightleaf)) )
            {
                if (rightparent == parent)
                    btree_i_delete_merge_result(&myres, btree_i_shift_left_leaf(leaf, rightleaf, rightparent, parentslot));
                else
                    btree_i_delete_merge_result(&myres, btree_i_merge_leaves(tree, leftleaf, leaf, leftparent));
            }
            ...
            {
                if (leftparent == parent)
                    btree_i_shift_right_leaf(leftleaf, leaf, leftparent, parentslot - 1);
                else
                    btree_i_delete_merge_result(&myres, btree_i_merge_leaves(tree, leaf, rightleaf, rightparent));
            }
    ...
    else // !curr->btree_i_is_leaf()
    {
        btree_i_inner_node *inner = (btree_i_inner_node*)(curr);
        btree_i_inner_node *leftinner = (btree_i_inner_node*)(left);
        btree_i_inner_node *rightinner = (btree_i_inner_node*)(right);

        // find first slot below which the searched iterator might be
        // located.

        btree_c_result result;
        int slot = btree_i_find_lower_inner(inner, key-1e-8);

        while (slot <= inner->num_slots_used)
        {
            void *myleft, *myright;
            btree_i_inner_node *myleftparent, *myrightparent;

            if (slot == 0) {
                myleft = (left == NULL) ? NULL : ((btree_i_inner_node*)(left))->slot_ptrs[((btree_i_inner_node*)left)->num_slots_used - 1];
            ...
            if (slot == inner->num_slots_used) {
                myright = (right == NULL) ? NULL : ((btree_i_inner_node*)(right))->slot_ptrs[0];
            ...
            result = btree_i_delete_descend(tree, key, value,
                                        inner->slot_ptrs[slot],
                                        myleft, myright,
                                        myleftparent, myrightparent,
                                        inner, slot);

            if (!btree_i_result_has(result, btree_not_found))
                break;

            // continue recursive search for leaf on next slot

            if (slot < inner->num_slots_used && inner->slot_keys[slot]>key+1e-8)
                return btree_c_result_of(btree_not_found, 0);
        ...
        if (btree_i_result_has(result, btree_update_last_key))
        {
            if (parent && parentslot < parent->num_slots_used)
            {
                assert(parent->slot_ptrs[parentslot] == curr);
            ...
            {
                btree_i_delete_merge_result(&myres, btree_c_result_of(btree_update_last_key, result.last_key));
            }
        }

        if (btree_i_result_has(result, btree_fix_merge))
        {
            // either the current node or the next is empty and should be removed
            if (btree_i_num_slots_used(inner->slot_ptrs[slot]) != 0)
                slot++;

            // this is the child slot invalidated by the merge
            assert(btree_i_num_slots_used(inner->slot_ptrs[slot]) == 0);

            btree_i_free_node(inner->slot_ptrs[slot]);

            btree_i_copy_keys_fwd(inner->slot_keys + slot, inner->slot_keys + inner->num_slots_used,
                      inner->slot_keys + slot-1);
            btree_i_copy_child_fwd(inner->slot_ptrs + slot+1, inner->slot_ptrs + inner->num_slots_used+1,
                      inner->slot_ptrs + slot);
            ...
            if (inner->level == 1)
            {
                ...
                slot--;
                btree_i_leaf_node *child = (btree_i_leaf_node*)(inner->slot_ptrs[slot]);
                inner->slot_keys[slot] = child->slot_keys[ child->num_slots_used-1 ];
            }
        }

        if (btree_i_is_below_min(tree, inner) && !(inner == tree->root && inner->num_slots_used >= 1))
        {
            ...
            // a shift, then merging is necessary. choose the more local merger
            // with our id
            else if ( (leftinner == NULL || btree_i_is_few(tree, leftinner)) && (rightinner == NULL || btree_i_is_few(tree, rightinner)) )
            {
                if (leftparent == parent)
                    btree_i_delete_merge_result(&myres, btree_i_merge_inner(leftinner, inner, leftparent, parentslot - 1));
                else
                    btree_i_delete_merge_result(&myres, btree_i_merge_inner(inner, rightinner, rightparent, parentslot));
            }
            // case : the right leaf has extra data, so balance right with current
            else if ( (leftinner != NULL && btree_i_is_few(tree, leftinner)) && (rightinner != NULL && !btree_i_is_few(tree, rightinner)) )
            {
                if (rightparent == parent)
                    btree_i_shift_left_inner(inner, rightinner, rightparent, parentslot);
                else
                    btree_i_delete_merge_result(&myres, btree_i_merge_inner(leftinner, inner, leftparent, parentslot - 1));
            }
            // case : the left leaf has extra data, so balance left with current
            ...
            {
                if (leftparent == parent)
                    btree_i_shift_right_inner(leftinner, inner, leftparent, parentslot - 1);
                else
                    btree_i_delete_merge_result(&myres, btree_i_merge_inner(inner, rightinner, rightparent, parentslot));
            }
            // case : both the leaf and right leaves have extra data and our
...
static btree_c_result btree_i_merge_leaves(btree_i* const tree, btree_i_leaf_node* left, btree_i_leaf_node* right, btree_i_inner_node* parent)
{
    ...
    assert(btree_i_is_leaf(left) && btree_i_is_leaf(right));
    assert(parent->level == 1);

    assert(left->num_slots_used + right->num_slots_used < tree -> leaf_max_num_slots);

    btree_i_copy_keys_fwd(right->slot_keys, right->slot_keys + right->num_slots_used,
              left->slot_keys + left->num_slots_used);
    btree_i_copy_data_fwd(right->slot_data, right->slot_data + right->num_slots_used,
...
static btree_c_result btree_i_merge_inner(btree_i_inner_node* left, btree_i_inner_node* right, btree_i_inner_node* parent, unsigned int parentslot)
{
    ...
    btree_i_copy_keys_fwd(right->slot_keys, right->slot_keys + right->num_slots_used,
              left->slot_keys + left->num_slots_used);
    btree_i_copy_child_fwd(right->slot_ptrs, right->slot_ptrs + right->num_slots_used+1,
...
static btree_c_result btree_i_shift_left_leaf(btree_i_leaf_node *left, btree_i_leaf_node *right, btree_i_inner_node *parent, unsigned int parentslot)
{
    ...
    btree_i_copy_keys_fwd(right->slot_keys, right->slot_keys + shiftnum,
              left->slot_keys + left->num_slots_used);
    btree_i_copy_data_fwd(right->slot_data, right->slot_data + shiftnum,
              left->slot_data + left->num_slots_used);
    ...
    btree_i_copy_keys_fwd(right->slot_keys + shiftnum, right->slot_keys + right->num_slots_used,
              right->slot_keys);
    btree_i_copy_data_fwd(right->slot_data + shiftnum, right->slot_data + right->num_slots_used,
              right->slot_data);
...
static void btree_i_shift_left_inner(btree_i_inner_node *left, btree_i_inner_node *right, btree_i_inner_node *parent, unsigned int parentslot)
{
    ...
    btree_i_copy_keys_fwd(right->slot_keys, right->slot_keys + shiftnum-1,
              left->slot_keys + left->num_slots_used);
    btree_i_copy_child_fwd(right->slot_ptrs, right->slot_ptrs + shiftnum,
              left->slot_ptrs + left->num_slots_used);
    ...
    btree_i_copy_keys_fwd(right->slot_keys + shiftnum, right->slot_keys + right->num_slots_used,
              right->slot_keys);
    btree_i_copy_child_fwd(right->slot_ptrs + shiftnum, right->slot_ptrs + right->num_slots_used+1,
              right->slot_ptrs);
...
static void btree_i_shift_right_leaf(btree_i_leaf_node *left, btree_i_leaf_node *right, btree_i_inner_node *parent, unsigned int parentslot)
{
    ...
    btree_i_copy_keys_bwd(right->slot_keys, right->slot_keys + right->num_slots_used,
                       right->slot_keys + right->num_slots_used + shiftnum);
    btree_i_copy_data_bwd(right->slot_data, right->slot_data + right->num_slots_used,
                       right->slot_data + right->num_slots_used + shiftnum);

    right->num_slots_used += shiftnum;

    // copy the last items from the left node to the first slot in the right node.
    btree_i_copy_keys_fwd(left->slot_keys + left->num_slots_used - shiftnum, left->slot_keys + left->num_slots_used,
              right->slot_keys);
    btree_i_copy_data_fwd(left->slot_data + left->num_slots_used - shiftnum, left->slot_data + left->num_slots_used,
              right->slot_data);
...
static void btree_i_shift_right_inner(btree_i_inner_node *left, btree_i_inner_node *right, btree_i_inner_node *parent, unsigned int parentslot)
{
    ...
    btree_i_copy_keys_bwd(right->slot_keys, right->slot_keys + right->num_slots_used,
                       right->slot_keys + right->num_slots_used + shiftnum);
    btree_i_copy_child_bwd(right->slot_ptrs, right->slot_ptrs + right->num_slots_used+1,
    ...
    btree_i_copy_keys_fwd(left->slot_keys + left->num_slots_used - shiftnum+1, left->slot_keys + left->num_slots_used,
              right->slot_keys);
    btree_i_copy_child_fwd(left->slot_ptrs + left->num_slots_used - shiftnum+1, left->slot_ptrs + left->num_slots_used+1,
              right->slot_ptrs);
...
{
    btree_i_search_res ret = {NULL, 0};
    void *n = tree->root;
    if (!n) return ret;

    while(!btree_i_is_leaf(n))
    {
        const btree_i_inner_node *inner = (btree_i_inner_node*)(n);
        int slot = btree_i_find_lower_inner(inner, key);
...
void btree_i_dump(btree_i* const tree){
    ...
    while(l<=r){
        ...
        if(btree_i_is_leaf(q[l]))
        {
            printf("[  ");
            btree_i_leaf_node * p = (btree_i_leaf_node*)q[l];
            for(int i=0;i<p->num_slots_used;i++)
                printf("<%lf %d> ", *(p->slot_keys+i), *(p->slot_data+i));
            printf(" ]  ");
        }
        else
        {
            btree_i_inner_node* p = (btree_i_inner_node*)q[l];
            printf("[  ");
            for(int i=0;i<p->num_slots_used;i++){
