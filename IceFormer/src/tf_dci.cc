...
query_field_of_view
nearest_neighbour_ids: A matrix of shape (num of queries) x (num of neighbours) containing the cell_indices of the nearest neighbours to each query.
...
class DCIKNNOp : public OpKernel {
    ...
    void Compute(OpKernelContext* context) override {
        ...
        float construction_prop_to_visit = construction_prop_to_visit_tensor.flat<float>()(0);
        float construction_prop_to_retrieve = construction_prop_to_retrieve_tensor.flat<float>()(0);
        int construction_field_of_view = construction_field_of_view_tensor.flat<int32>()(0);
        float query_prop_to_visit = query_prop_to_visit_tensor.flat<float>()(0);
        float query_prop_to_retrieve = query_prop_to_retrieve_tensor.flat<float>()(0);
        int query_field_of_view = query_field_of_view_tensor.flat<int32>()(0);
        
        auto data = data_tensor.flat<float>().data();  // Tensor.flat() returns an Eigen::Tensor object (whose docs are available at https://eigen.tuxfamily.org/dox-devel/unsupported/eigen_tensors.html)
        auto query = query_tensor.flat<float>().data();
        ...
        if (update_db) {
            ...
            dci_db_mutex.lock();
            dci_reset(&dci_db);
            dci_add(&dci_db, dim, num_points, (float*) data, num_levels, construction_query_config);
            dci_db_mutex.unlock();
        ...
        float** nearest_neighbour_dists = (float **)malloc(sizeof(float *)*num_queries);
